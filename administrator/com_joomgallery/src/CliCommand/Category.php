...
//    $this->addOption('search', 's', InputOption::VALUE_OPTIONAL, Text::_('COM_JOOMGALLERY_CLI_CONFIG_SEARCH'));
  ...
  Usage: <info>php %command.full_name%</info>
    * You need to give a id of category using the <info>--id</info> option. Otherwisi ti will be requested
...
//    $this->ioStyle->title(Text::_('COM_JOOMGALLERY_CLI_ITEMS_LIST_DESC'));
    ...
    if (empty ($imageId)){
      $this->ioStyle->error("The category id '" . $imageId . "' is invalid (empty) !");

      return Command::FAILURE;
    }

    $categoryAssoc = $this->getItemAssocFromDB($imageId);

    // If no images are found show a warning and set the exit code to 1.
    if (empty($categoryAssoc))
    {
      $this->ioStyle->error("The category id '" . $imageId . "' is invalid, No image found matching your criteria!");
...
//      }
  ...
  private function getItemAssocFromDB(string $categoryId): array
  {
    ...
    $query
      ...
      $query->where($db->quoteName('id') . ' = ' . (int) $categoryId);
